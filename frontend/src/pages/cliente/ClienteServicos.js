...
const ClienteServicos = () => {
  ...
  // CORRIGIDO: Pagamento parcial agora é true por padrão
  ...
  const abrirModalAgendar = async (servico) => {
    try {
      ...
      if (response.data.length === 0) {
          ...
          setShowModal(false); // Fecha o modal se não houver profissionais
          return;
      }
      ...
      setProfissionalId('');
      setDataHora('');
      setPagamentoParcial(true); // Garante que esteja true ao abrir
    } catch (error) {
