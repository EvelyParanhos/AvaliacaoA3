...
import { agendamentoAPI } from '../../services/api'; // Usamos a API de agendamento
...
const AdminHistorico = () => {
  ...
  const [filtro, setFiltro] = useState(''); // Estado para o filtro de status
  ...
  const carregarHistorico = async () => {
    try {
      setLoading(true);
      // Chama a API de histórico, passando o filtro (ou string vazia para todos)
      const response = await agendamentoAPI.listarHistorico(filtro);
      
      // Ordena pela data mais recente (o backend já deve fazer isso, mas garantimos)
